package com.example.userapplication.authStrategy;
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain chain)
        ...
        if (requestTokenHeader != null && requestTokenHeader.startsWith(Constants.BEARER)) {
            ...
            try {
                ...
                response.sendError(HttpServletResponse.SC_UNAUTHORIZED, Constants.JWT_NOT_FOUND);
            } catch (ExpiredJwtException e) {
                response.sendError(HttpServletResponse.SC_UNAUTHORIZED, Constants.JWT_EXPIRED);
            }
        } else {
            logger.warn(Constants.JWT_WARN);
        }
